...
class SoftDQNAgent(DQNAgentBase):
    ...
    def action(self, state, deterministic=False):
        self.net.eval()
        state = torch.from_numpy(state).float().reshape(-1, self.net.obs_dim).to(self.net.device)
    ...
    def td_target(self, reward, next_state, terminated, n):
        next_q1 = symexp(self.net(next_state))
        next_q2 = symexp(self.target_net(next_state))
        next_q = torch.minimum(next_q1, next_q2)
        logsumexp = self.alpha * torch.logsumexp(next_q / self.alpha, dim=1).reshape(-1, 1)
...
if __name__ == "__main__":
    ...
    config = Config()
    agent = SoftDQNAgent('test', Q, config)
    # agent.load()
    agent.n_step = 5
    reward_container = []
    Loss = []
