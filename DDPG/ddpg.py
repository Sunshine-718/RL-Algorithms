...
class DDPG(NetworkBase):
    def __init__(self, lr, obs_dim, h_dim, action_dim, action_limit=1., dropout=0., computes_grad=True, device='cpu'):
        ...
        self.pi = nn.Sequential(nn.Linear(obs_dim, h_dim),
                                ...
                                nn.Tanh())
        self.q = nn.Sequential(nn.Linear(obs_dim + action_dim, h_dim),
                               nn.LayerNorm(h_dim),
                               nn.Dropout(dropout),
                               nn.SiLU(True),
                               nn.Linear(h_dim, h_dim),
...
class DDPGAgent(AgentBase):
    def __init__(self, name, ac, config):
        ...
        self.buffer = ReplayBuffer(ac.obs_dim, config.capacity, self.net.action_dim, config.discount, config.n_step, ac.device)
...
if __name__ == "__main__":
    ...
    ac = DDPG(1e-3, env.observation_space.shape[0], 128, env.action_space.shape[0], 1, 0, True, device)
    config = Config()
