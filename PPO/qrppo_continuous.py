...
class PPOAgent(PPOAgentBase):
    def __init__(self, name, net, config: Config):
        ...
        self.qr_tau = torch.linspace(0.5 / self.num_quantiles, 1 - 0.5 / self.num_quantiles, self.num_quantiles).to(self.device).view(1, -1)
    ...
    def step(self, batch_size=64):
        ...
        for _ in range(self.epoch):
            for state, action, old_logp, adv, td_targ in loader:
                ...
                critic_loss = quantile_huber_loss(new_value, td_targ, self.qr_tau)
...
if __name__ == "__main__":
    ...
    config = Config()
    agent = PPOAgent('inverted_pendulum_qrppo_continuous', ac, config)
    agent.normalizers = {'reward': reward_normalizer}
